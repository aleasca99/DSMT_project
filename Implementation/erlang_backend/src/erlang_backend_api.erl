...
handle_call({final_solution, EventId, FinalSolution}, _From, State) ->
    ...
    %% Example: notify_java(EventId, FinalSolution)
    {noreply, State};
...
%% Placeholder function to notify the Java backend about a node status change.
notify_java(Node, Status) ->
    io:format("Notifying Java: Node ~p is ~p~n", [Node, Status]).
